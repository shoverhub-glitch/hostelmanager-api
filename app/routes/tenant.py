...
async def create_tenant(request: Request, tenant: TenantCreate):
    try:
        ...
        # Check tenant quota per property
        count = await db["tenants"].count_documents({
            "propertyId": tenant.propertyId
        })
        if count >= settings.MAX_TENANTS_PER_PROPERTY:
            raise HTTPException(
                status_code=402,
                detail=f"You can only add {settings.MAX_TENANTS_PER_PROPERTY} tenants per property."
            )
