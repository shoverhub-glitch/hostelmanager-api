...
async def create_property(request: Request, property: PropertyCreate):
    try:
        ...
        # Check property quota
        count = await db["properties"].count_documents({
            "ownerId": user_id
        })
        if count >= settings.MAX_PROPERTIES_PER_OWNER:
            raise HTTPException(
                status_code=402,
                detail=f"You can only create {settings.MAX_PROPERTIES_PER_OWNER} properties. Delete an existing property to add more."
            )
